...
服务端提供 RA2（安全类型 5）时，网页通过 Python 代理完成 RSA 密钥交换、
密码认证以及后续桌面数据的 AES-EAX 加解密，无需修改目标服务器。
连接测试也会实际验证 RA2 密码并读取桌面信息。仅支持密码认证的网页输入；
要求系统用户名的服务器会明确提示缺少用户名。

网页通过 WebSocket 首条消息向代理传递密码，不把密码放入 URL。代理仅在当前
连接中使用凭据。代理到目标的 RA2 会话加密，浏览器到代理的链路需由 HTTPS/WSS
或可信内网保护。当前不保存或校验服务器公钥指纹，部署边界仍以可信内网/VPN 为准。
仅提供其他 RealVNC 私有类型、但没有类型 5 的服务器不在本次兼容范围内。
